...
class GeodesicAlgorithmSubdivision : public GeodesicAlgorithmGraphBase<SubdivisionNode>
{
  ...
  public:
    ...
    void set_subdivision_level(unsigned subdivision_level)
    {
        ...
        m_nodes.reserve(m_mesh->vertices().size() + m_mesh->edges().size() * subdivision_level);
  ...
  private:
    void list_nodes(MeshElementBase* p, // list nodes that belong to this mesh element
                    std::vector<node_pointer>& storage,
                    double threshold_distance = -1.0); // list only the nodes whose current distance
                                                       // is larger than the threshold

    unsigned m_subdivision_level; // when level is equal to 1, this algorithm corresponds to the
                                  // Dijkstra algorithm
};
...
void
GeodesicAlgorithmSubdivision::list_nodes_visible_from_source(MeshElementBase* p,
                                                             std::vector<node_pointer>& storage)
{
    assert(p->type() != UNDEFINED_POINT);
...
void
GeodesicAlgorithmSubdivision::list_nodes_visible_from_node(
  node_pointer node, // list all nodes that belong to this mesh element
  std::vector<node_pointer>& storage,
  std::vector<double>& distances,
  double threshold_distance)
