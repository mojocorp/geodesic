...
GEODESIC_DLL_IMPORT long
distance_and_source_for_all_vertices(
...
GEODESIC_DLL_IMPORT long
trace_back(long algorithm_id, double* destination, double** path)
{
    geodesic::SurfacePoint point;
    geodesic::GeodesicAlgorithmBase* algorithm = algorithms[algorithm_id].get();
    fill_surface_point_structure(point, destination, algorithm->mesh());
...
GEODESIC_DLL_IMPORT void
propagate(long algorithm_id,
         ...
         double** edges)
{
    ...
    new_mesh->initialize_mesh_data(num_points, points, num_triangles, triangles);
    ...
    *edges = output_buffer.get<double>();
    ...
    for (std::size_t i = 0; i < *num_edges; ++i) {
        ...
        buffer[3] = edge.adjacent_faces().size() == 2 ? edge.adjacent_faces()[1]->id() : -1;
    }
...
GEODESIC_DLL_IMPORT long
new_algorithm(long mesh_id, long type, long subdivision)
{
...
GEODESIC_DLL_IMPORT void
delete_mesh(long id) // delete mesh and all related algorithms
{
