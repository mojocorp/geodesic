...
extern "C"
{
    ...
    GEODESIC_DLL_IMPORT long new_algorithm(
    ...
    GEODESIC_DLL_IMPORT long trace_back(
      long algorithm_id, // using procomputed distance field, compute a shortest path from
                         // destination to the closest source
      double* destination,
      double** path);

    GEODESIC_DLL_IMPORT long distance_and_source(
      long algorithm_id, // quickly find what source this point belongs to and what is the distance
                         // to this source
      double* destination,
